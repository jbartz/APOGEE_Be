...
for (i,fname) in enumerate(ls_period):
#for (i,fname) in enumerate(ls_period[:4]):
    data1 = np.genfromtxt('/media/sf_Astro/KELT/jon_lcs/tfa/Sigma_Ori_E{0}'.format(ls_period[i]),usecols=2) #data for phase folded plot with the best periods
    data1 = data1[1:]
    p_1 = data1[0]
    p_2 = data1[3]
    p_3 = data1[6]
    ...
    data2 = np.genfromtxt('//media/sf_Astro/KELT/jon_lcs/tfa/Sigma_Ori_E/{0}/{1}'.format(abenums[i],ls_periodogram[i])) #data for periodogram
    freq = data2[:,0]
    power = data2[:,1]
    period = 1/freq

    lc_data_comb = np.genfromtxt('//media/sf_Astro/KELT/jon_lcs/tfa/west/{0}'.format(kelt_id[i])) #data for lightcurve (time and magnitude)
...
#------------------end averaging stuff--------------------
    ...
    f.suptitle(abenums[i] + '  LS Periodogram' + '  subsample= ' + subsample + '  whitened with ' + sigma + ' sigma clipping' \
               +  '   Trial ' + trial + '\n' + 'Min Period = ' + minperiod + '   Max Period = ' + maxperiod + '\n')

    ax1.scatter(time/1000., mag, marker=',',s=0.15)
...
#    plt.ylim([mag.max() + 0.01, mag.min() - 0.01])
    ...
    plt.savefig('/media/sf_Astro/KELT/jon_lcs/tfa/Sigma_Ori_E/{0}/phased_lcs_ls/{0}_{1}.png'.format(abenums[i], trial))
    plt.cla()
    plt.clf()
