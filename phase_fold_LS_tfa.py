...
ls_periodogram = [ f for f in listdir('/Volumes/ext_drive/work/KELT/jon_lcs/tfa/LS/periodograms/{0}'.format(trial)) if isfile(join('/Volumes/ext_drive/work/KELT/jon_lcs/tfa/LS/periodograms/{0}'.format(trial),f))][1:]

ls_period = [ f for f in listdir('/Volumes/ext_drive/work/KELT/jon_lcs/tfa/LS/peaks/{0}'.format(trial)) if isfile(join('/Volumes/ext_drive/work/KELT/jon_lcs/tfa/LS/peaks/{0}'.format(trial),f))][1:]
...
#for (i,fname) in enumerate(ls_period[:4]):
    fname = f[:-3]
    data1 = np.genfromtxt('/Volumes/ext_drive/work/KELT/jon_lcs/tfa/LS/peaks/{0}/{1}.txt'.format(trial,fname),usecols=2) #data for phase folded plot with the best periods
    data1 = data1[1:]
    p_1 = data1[0]
    p_2 = data1[3]
    p_3 = data1[6]
    ...
    data2 = np.genfromtxt('/Volumes/ext_drive/work/KELT/jon_lcs/tfa/LS/periodograms/{0}/{1}.ls'.format(trial,fname)) #data for periodogram
    freq = data2[:,0]
    power = data2[:,1]
    period = 1/freq
...
#------------------end averaging stuff--------------------
    ...
    f.suptitle('  LS Periodogram' + '  subsample= ' + subsample + '  whitened with ' + sigma + ' sigma clipping' \
...
#    plt.ylim([mag.max() + 0.01, mag.min() - 0.01])
    ...
    plt.savefig('/Volumes/ext_drive/work/KELT/jon_lcs/tfa/LS/figs/{0}/{1}.png'.format(trial,fname))
    plt.cla()
    plt.clf()
